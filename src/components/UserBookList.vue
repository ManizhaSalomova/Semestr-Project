      ...
      <v-row align="center" justify="center">
        ...
        <v-col  cols="12" md="10" lg="10" align="center" justify="center">
          <v-alert v-if="showMsg === 'new'"
                   ...
                   type="success"
          >
            New userBook has been added.
          </v-alert>
          <v-alert v-if="showMsg === 'update'" dismissible
                   :value="true"
                   type="success"
          >
           userBook information has been updated.
          </v-alert>
          <v-alert v-if="showMsg === 'deleted'" dismissible
                   :value="true"
                   type="success"
          >
            Selected UserBook has been deleted.
          </v-alert>
        </v-col>
      </v-row>
        ...
        <v-col cols="12" md="5" v-resize="onResize">
            ...
            >
                    <template v-slot:item="props">
                      <tr>
                        <td align="left">{{ props.item.book }}</td>
                        <td align="left">{{ props.item.status }}</td>
                        <td nowrap="true" align="left">{{ props.item.rating }}</td>
                        <td align="center"><v-icon @click="updateUserBook(props.item)">mdi-pencil</v-icon></td>
                        <td align="center"><v-icon @click="deleteUserBook(props.item)">mdi-delete</v-icon></td>
                      </tr>  
                    </template>
              </v-data-table>
              <v-data-iterator 
                    ...
                    >
                      <v-card>
                        <v-card-title class="pb-0 pt-0 pl-0">
                          <v-col cols="9" class="text-left body-2 text-truncate">{{ item.book }}</v-col>
                          <v-col cols="3" class="text-center">
                            <v-card-actions>
                              <v-icon @click="updateUserBook(item)" class="small">mdi-pencil</v-icon>
                              <v-icon @click="deleteUserBook(item)" class="small">mdi-delete</v-icon>
              ...
              <v-btn class=" mt-4 " @click="addNewUserBook">Add UserBook</v-btn>  
        </v-col>  
      </v-row>
    </v-container>  
  </v-main>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getUserBooks() {
        apiService.getUserBookList().then(response => {
          this.userBooks = response.data.data;
          this.userBookSize = this.userBooks.length;
      ...
      addNewUserBook() {
        if (localStorage.getItem("isAuthenticates")
          && JSON.parse(localStorage.getItem("isAuthenticates")) === true) {
          router.push('/userBook-create');
        } else {
          router.push("/auth");
        }
      },
      updateUserBook(userBook) {
        router.push('/userBook-create/' + userBook.pk);
      },
      deleteUserBook(userBook) {
        apiService.deleteUserBook(userBook.pk).then(response => {
          if (response.status === 204) {
            router.push('/userBook-list/deleted/')
            this.getUserBooks()
          }
        }).catch(error => {
          if (error.response.status === 401) {
